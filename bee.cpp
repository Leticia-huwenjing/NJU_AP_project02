...
Bee::Bee(QWidget* parent, int time, int _x, int _y)
    ...
    connect(posTimer, &QTimer::timeout, this, [=](){
        if (isAlive) {
            ...
            if (isSlowed && !slowTimer->isActive()) {
                slowTimer->setInterval(3000);
                slowTimer->start(3000);
                cartoon->stop();
                cartoon->setStartValue(QRect(this->x(), this->y(), 80, 80));
                cartoon->setDuration(30000*((double)this->x())/(begin_y*80));
    ...
    connect(slowTimer, &QTimer::timeout, this, [=](){
        ...
        cartoon->stop();
        cartoon->setStartValue(QRect(this->x(), this->y(), 80, 80));
        cartoon->setDuration(10000*((double)this->x())/(begin_y*80));
        cartoon->setEndValue(QRect(0, this->y(), 80, 80));
        cartoon->start();
    });
}
